...
from mistral_llm import Mistral_LLM

class Mistral_Chatbot:
    """
    A chatbot class that manages conversations using Mistral's language models.

    This class provides functionality to maintain conversation history, manage multiple
    conversations, and interact with the Mistral API through the Mistral_LLM class.

    Attributes:
        _chatbot_counter (int): Class variable to track number of chatbot instances
        llm (Mistral_LLM): Instance of the Mistral LLM interface
    ...
    def __init__(
        ...
        """
        Mistral_Chatbot._chatbot_counter += 1
        self.llm = llm
        self.system_prompt = system_prompt
        self.verbose = verbose
        self.chatbot_id = Mistral_Chatbot._chatbot_counter
        self.name = name or f"mistral_chatbot_{self.chatbot_id}"
    ...
    def _create_conversation_folder(self) -> str:
        ...
        chatbot_folder = f"{base_folder}/mistral/{self.name}"
